/* Media Controller — frontend logic */

const PROJECT = 'media_controller';
let polling = null;
...
function stopProject() {
  ...
  document.getElementById('muteLabel').textContent = 'No';
    ...
    fetch(`/status/${PROJECT}`)
      ...
      .then(data => {
        if (data.status === 'active') {
          ...
          const vol = data.volume !== undefined ? data.volume : 0;
          document.getElementById('volLabel').textContent = vol;
          document.getElementById('volBar').style.width = vol + '%';
          document.getElementById('muteLabel').textContent = data.muted ? 'Yes' : 'No';
        }
      })
      .catch(() => {});
  }, 300);
